...
for url in urls:
    ...
    for p in soup.find_all('p'):
        p_text, seen_emails = contains_email(p.text, seen_emails)
        p.string = p_text
...
# Convert the sets and lists to strings
unique_data_str = "\n".join(unique_data)

# Save the data to a text file
# Save the data to a text file with explicit encoding
with open('output-miraloma-four-ten.txt', 'w', encoding='utf-8') as file:
    # Encode non-ASCII characters in a way compatible with utf-8
    unique_data_str_encoded = unique_data_str.encode('utf-8', 'ignore').decode('utf-8')
    seen_emails_encoded = '\n'.join(seen_emails).encode('utf-8', 'ignore').decode('utf-8')

    file.write(f"Unique Data:\n{unique_data_str_encoded}\n\nEmail Addresses:\n{seen_emails_encoded}\n")